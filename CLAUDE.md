...
- **2026-06-29**: A11y follow-ups (alt text + marquee SR):
  - **Marquee accessibility (final pattern)** — removed `aria-hidden` from the `.marquee` container; the **first** `.marquee__text` copy is the readable, interactive one for screen readers. The 3 visual duplicates (needed for the seamless CSS scroll) carry `aria-hidden="true"` **and `inert`** — `inert` removes them from the tab order and a11y tree so any markdown-generated links inside aren't focusable (an `aria-hidden` region must not contain focusable elements). Net: the banner is announced once, with its links interactive in copy 1 only.
