...
- **2026-06-29**: A11y follow-ups (alt text + marquee SR):
  - **Marquee now announced to screen readers** — removed `aria-hidden="true"` from the `.marquee` container in `banner.njk`. (Per follow-up request, `aria-hidden` was also kept off the individual `.marquee__text` copies — so the banner message is announced for each of the 4 visual copies; accepted by decision.)
