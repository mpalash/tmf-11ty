...
- **2026-06-29**: A11y follow-ups (alt text + marquee SR):
  - **Marquee now announced to screen readers** — removed `aria-hidden="true"` from the `.marquee` container in `banner.njk`. To avoid the banner being read 4× (the track holds 4 visual copies for the seamless CSS scroll), the 2nd–4th `.marquee__text` copies are marked `aria-hidden="true"`, so the message is announced exactly once.
